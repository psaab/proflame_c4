...
- **Version**: 2.0
- **Date**: May 2026
- **Driver Version**: 2026061601 (2026-06-16)
...
|---------|------|---------|
| 2026061601 | 2026-06-16 | **Fix: connection dropped by the device every ~30s** (continuous `WebSocket close frame received from device` → `Connection Lost (4)` → reconnect loop). C1 Phase 2 (#68) replaced the hand-rolled 5s `PROFLAMEPING` text-frame keepalive with the vendored 30s RFC 6455 WS-level ping; on-device the Proflame dongle (FW 625.04.673) closes the socket in lock-step with that ping firing (~30s after each connect) — it enforces an inbound-idle session timeout and does not treat the WS control-frame ping as activity. Fix (B4/#86): disable the WS-level ping (`Connect` sets `ws.ping_interval = 0`; `vendor/.../websocket.lua` now guards `ping_interval > 0` before arming the Ping timer) and restore the app-level `PROFLAMEPING` keepalive on a new **`Keepalive Interval (seconds)`** property (default 15, 0 disables). `gMissedKeepalives` is a half-open-link watchdog: any inbound frame resets it, three silent intervals force a reconnect. **Static surface change:** adds the `Keepalive Interval (seconds)` property — Director reload / re-add picks it up; existing installs get the 15s default. |
